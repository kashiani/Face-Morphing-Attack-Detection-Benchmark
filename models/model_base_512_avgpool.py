...
class MorphDetection(nn.Module):
    ...
    def forward(self, x):
        ...
        res_out = self.avgpool(self.Feature_extraction_AlexNet(residual) ) # batchsize x 256 x 6 x 6 for 512 or batchsize x 15 x 15
        res_out_flat = res_out.view(res_out.size(0), 256 * 6 * 6) # batchsize x 9216
        res_out_fc6 = self.Feature_classifier_AlexNet(res_out_flat)  # batchsize x 2
